...
class ProductController {
    ...
    async getCategoryOrder(req, res) {
        try {
            // 利用 ConfigReader 讀取設定
            const { configReader } = getServices(req);
            const systemConfig = await configReader.getSystemConfig();
    ...
    async saveCategoryOrder(req, res) {
        try {
            ...
            const { configWriter } = getServices(req);
            
            // 呼叫 ConfigWriter 寫入 (Key: PRODUCT_CATEGORY_ORDER, Value: JSON string)
            await configWriter.updateSystemPref('PRODUCT_CATEGORY_ORDER', JSON.stringify(order));
            
            // 清除 ConfigReader 的快取，確保下次讀取是新的
            const { configReader } = getServices(req);
            if(configReader.cache['systemConfig']) delete configReader.cache['systemConfig'];
