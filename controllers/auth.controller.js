...
class AuthController {
    ...
    async login(req, res) {
        try {
            ...
            // 注意：現在主要依賴 AuthReader，Config 內通常不存使用者
            let allUsers = config.SYSTEM_USERS;
            
            if (!allUsers || allUsers.length === 0) {
                 const { authReader } = req.app.get('services');
                 allUsers = await authReader.getUsers();
            }
    ...
    async verifyPassword(req, res) {
        try {
            ...
            // 強制刷新快取以確保資料最新
            if (authReader.cache && authReader.cache['users']) {
                delete authReader.cache['users'];
            }
            
            const allUsers = await authReader.getUsers();
    ...
    async changePassword(req, res) {
        try {
            const { authReader, authWriter } = req.app.get('services');
            ...
            await authWriter.updatePassword(user.rowIndex, newHash);
