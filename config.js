...
module.exports = {
    ...
    // ★★★ 【新增】系統設定專用 Sheet ID ★★★
    SYSTEM_SETTING_SPREADSHEET_ID: process.env.SYSTEM_SETTING_SPREADSHEET_ID,
    ...
    validateConfig: function() {
        const missing = [];
        if (!this.SPREADSHEET_ID) missing.push('SPREADSHEET_ID');
        if (!this.AUTH_SPREADSHEET_ID) missing.push('AUTH_SPREADSHEET_ID');
        if (!this.SYSTEM_SETTING_SPREADSHEET_ID) missing.push('SYSTEM_SETTING_SPREADSHEET_ID');
        if (!this.MARKET_PRODUCT_SHEET_ID) missing.push('MARKET_PRODUCT_SHEET_ID');
        
        if (missing.length > 0) {
            console.error('❌ [Config] 嚴重錯誤：缺少必要環境變數:', missing.join(', '));
