...
class WeeklyBusinessService {
    constructor(services) {
        ...
        // 【關鍵修正】這裡必須使用 configReader，因為 Service Container 是這樣命名的
        // 舊代碼: this.systemReader = services.systemReader;
        this.configReader = services.configReader;
        
        // 安全檢查
        if (!this.configReader) {
            console.error('🔥 [WeeklyBusinessService] 嚴重錯誤: 建構子未收到 configReader (請檢查 Service Container)');
        }
    ...
    async getWeeklyDetails(weekId) {
        ...
        // --- 1. 準備並行查詢 ---
        // 【關鍵修正】確保 configReader 存在才呼叫
        const configPromise = this.configReader 
            ? this.configReader.getSystemConfig() 
            : Promise.resolve({});

        const queries = [
            this.calendarService.getHolidaysForPeriod(firstDay, endQueryDate), // 0: 國定假日
            configPromise // 1: 系統設定 (包含篩選規則) - 改用 configReader
        ];
